...
`tansient' overrides `display-buffer-alist', so shackle rules for window
placements, which causes `shackle-rules' not being applied when commands are
executed via the `ef-deflang' menus. "
  (let ((doc
         (format "`display-buffer' wrapper for %s (generated by ef-deflang)"
                 fn))
        (wrap (intern (format "ef-%s-deflang-wrapper" fn))))
    (eval
